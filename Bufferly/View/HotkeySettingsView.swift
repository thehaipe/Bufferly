...
struct HotkeySettingsView: View {
    @State private var hotKeyService = HotKeyService.shared
        ...
        Form {
            Section {
                HStack {
                    ...
                    HStack(spacing: 5) {
                        ForEach(hotKeyService.modifierSymbols(for: hotKeyService.currentModifiers), id: \.self) { symbol in
                             KeycapView(symbol: symbol)
                        }
                        KeycapView(symbol: hotKeyService.keyString(for: hotKeyService.currentKeyCode))
                    }
                    ...
                    Button {
                        ...
                        Text(isRecording ? "Waiting for key presses" : "Click to change")
                            .font(.subheadline)
                            ...
                            .background(
                                RoundedRectangle(cornerRadius: 6)
                                    .fill(isRecording ? Color.clear : Color.secondary.opacity(0.1))
...
struct KeyMonitor: NSViewRepresentable {
    ...
    class Coordinator {
        ...
        func handle(event: NSEvent) {
            ...
            DispatchQueue.main.async {
                HotKeyService.shared.register(keyCode: keyCode, modifiers: carbonModifiers)
                self.parent.isRecording = false
            }
        }
